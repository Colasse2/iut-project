...
module.exports = [
    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                for (const user of users) {
                    await mailService.sendMoficationMovieNotificationEmail(user.mail, updatedMovie);
                }
    ...
    {
        method: 'POST',
        path: '/movie/{idUser}/{idMovie}/favorite',
        options: {
            tags: ['api'],
            auth: {
                scope: ['user', 'admin']
        ...
        handler: async (request, h) => {
            try {
                ...
                const user = await User.query().findById(idUser).withGraphFetched('favorites');
                if (!user) {
                    return h.response({ message: 'User not found' }).code(404);
                }

                const movie = await Movie.query().findById(idMovie);
                if (!movie) {
                    return h.response({ message: 'Movie not found' }).code(404);
                }

                if (user.favorites && user.favorites.some(fav => fav.id === idMovie)) {
                    return h.response({ message: 'Movie is already in your favorites' }).code(400);
                }

                await user.$relatedQuery('favorites').relate(idMovie);
    ...
    },
    {
        ...
        handler: async (request, h) => {
            try {
                ...
                const movieInFavorites = user.favorites && user.favorites.some(fav => fav.id === idMovie);
                if (!movieInFavorites) {
                    return h.response({ message: 'Movie not in your favorites' }).code(400);
                }

                await user.$relatedQuery('favorites').unrelate().where('movieId', idMovie);
