...
class Movie extends Model {
    ...
    $beforeInsert(queryContext) {
        ...
        this.updatedAt = currentDate.toISOString().split('T')[0] + ' ' + currentDate.toTimeString().split(' ')[0];
        this.createdAt = this.updatedAt;

        if (this.releaseDate) {
            if (this.releaseDate instanceof Date) {
                this.releaseDate = this.releaseDate.toISOString().split('T')[0];
            } else {
    ...
    $beforeUpdate(opt, queryContext) {
        this.updatedAt = new Date().toISOString().split('T')[0] + ' ' + new Date().toTimeString().split(' ')[0];
    }
}
